...
function build(fullClean) {
    if (fullClean && fs.existsSync(DIST)) {
    ...
    if (fs.existsSync(adminSrc)) copyDir(adminSrc, path.join(DIST, 'admin'));

    console.log(`[${new Date().toLocaleTimeString()}] Built all pages`);
}
    ...
    for (const file of files) {
        try {
            ...
            if (lastMtimes[file] !== mtime) {
                if (lastMtimes[file] !== undefined) changed = true;
